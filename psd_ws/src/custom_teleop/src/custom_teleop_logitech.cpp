...
class LogitechTeleopPublisher : public rclcpp::Node
{
public:
    LogitechTeleopPublisher() : Node("custom_teleop_gamepad")
    {
...
private:
    ...
    void run()
    {
        ...
        while (rclcpp::ok())
        {  
            int top_button_Y = readButtonState(6);
            if(top_button_Y && !prev_state_Y){   //debouncing
                max_torque += step_torque;
            }
            prev_state_Y = top_button_Y;

            int bottom_button_A = readButtonState(7);
            if(bottom_button_A && !prev_state_A){   //debouncing
                max_torque -= step_torque;
            }
            prev_state_A = bottom_button_A;

            int throttle = readAxisValue(2);
            double torque = mapValue(-throttle, axis_min, axis_max, 0, max_torque);

            int brake = readAxisValue(3);
            double brake_torque = mapValue(-brake, axis_min, axis_max, 0, max_torque);
            torque -= brake_torque;

            // std::cout << "R2:\t" << R2_value << std::endl << "L2:\t" << L2_value << std::endl << std::flush;

            int raw_axis_value_0 = readAxisValue(0);
            double steering_angle = mapValue(raw_axis_value_0, axis_min, axis_max, -max_steer, max_steer);
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<LogitechTeleopPublisher>();
    node->start();
    rclcpp::shutdown();
    return 0;
}
