#include "psd_vehicle_hardware_interfaces/psd_vehicle_system.hpp"
...
{
CallbackReturn psd_vehicleSystem::on_init(const hardware_interface::HardwareInfo& hardware_info)
{
  RCLCPP_INFO(rclcpp::get_logger("psd_vehicleSystem"), "Initializing");
  ...
  for (const hardware_interface::ComponentInfo& joint : info_.joints)
  {
    ...
    {
      RCLCPP_FATAL(rclcpp::get_logger("psd_vehicleSystem"), "Joint '%s' has %zu command interfaces found. 1 expected.",
    ...
    {
      RCLCPP_FATAL(rclcpp::get_logger("psd_vehicleSystem"), "Joint '%s' have %s command interfaces found. '%s' expected.",
    ...
    {
      RCLCPP_FATAL(rclcpp::get_logger("psd_vehicleSystem"), "Joint '%s' has %zu state interface. 2 expected.",
    ...
    {
      RCLCPP_FATAL(rclcpp::get_logger("psd_vehicleSystem"), "Joint '%s' have '%s' as first state interface. '%s' expected.",
    ...
    {
      RCLCPP_FATAL(rclcpp::get_logger("psd_vehicleSystem"), "Joint '%s' have '%s' as second state interface. '%s' expected.",
  ...
  {
    RCLCPP_INFO(rclcpp::get_logger("psd_vehicleSystem"), "Joint '%s' found", j.name.c_str());
  ...
  {
    RCLCPP_FATAL(rclcpp::get_logger("psd_vehicleSystem"), "Joint order size is invalid");
    return CallbackReturn::ERROR;
  }
  ...
  for (auto& j : info_.joints)
  {
    ...
    {
      RCLCPP_FATAL(rclcpp::get_logger("psd_vehicleSystem"), "Joint '%s' missing from velocity command joint order",
                   j.name.c_str());
      return CallbackReturn::ERROR;
    }
  }

  node_ = std::make_shared<rclcpp::Node>("psd_vehicle_system_node");
...
CallbackReturn psd_vehicleSystem::on_configure(const rclcpp_lifecycle::State&)
{
  RCLCPP_INFO(rclcpp::get_logger("psd_vehicleSystem"), "Configuring");
  return CallbackReturn::SUCCESS;
}

CallbackReturn psd_vehicleSystem::on_cleanup(const rclcpp_lifecycle::State&)
{
  RCLCPP_INFO(rclcpp::get_logger("psd_vehicleSystem"), "Cleaning up");
  return CallbackReturn::SUCCESS;
}

CallbackReturn psd_vehicleSystem::on_activate(const rclcpp_lifecycle::State&)
{
  RCLCPP_INFO(rclcpp::get_logger("psd_vehicleSystem"), "Activating");
  ...
  motor_state_subscriber_ =
      node_->create_subscription<JointState>("~/motors_response", rclcpp::SensorDataQoS(),
                                             std::bind(&psd_vehicleSystem::motor_state_cb, this, std::placeholders::_1));
  ...
  {
    RCLCPP_WARN(rclcpp::get_logger("psd_vehicleSystem"), "Feedback message from motors wasn't received yet");
...
CallbackReturn psd_vehicleSystem::on_deactivate(const rclcpp_lifecycle::State&)
{
  RCLCPP_INFO(rclcpp::get_logger("psd_vehicleSystem"), "Deactivating");
  cleanup_node();
  received_motor_state_msg_ptr_.set(nullptr);
  return CallbackReturn::SUCCESS;
}

CallbackReturn psd_vehicleSystem::on_shutdown(const rclcpp_lifecycle::State&)
{
  RCLCPP_INFO(rclcpp::get_logger("psd_vehicleSystem"), "Shutting down");
  cleanup_node();
  return CallbackReturn::SUCCESS;
}

CallbackReturn psd_vehicleSystem::on_error(const rclcpp_lifecycle::State&)
{
  RCLCPP_INFO(rclcpp::get_logger("psd_vehicleSystem"), "Handling error");
  cleanup_node();
  return CallbackReturn::SUCCESS;
}

std::vector<StateInterface> psd_vehicleSystem::export_state_interfaces()
{
...
std::vector<CommandInterface> psd_vehicleSystem::export_command_interfaces()
{
...
void psd_vehicleSystem::cleanup_node()
{
...
void psd_vehicleSystem::motor_state_cb(const std::shared_ptr<JointState> msg)
{
  RCLCPP_DEBUG(node_->get_logger(), "Received motors response");
  received_motor_state_msg_ptr_.set(std::move(msg));
}

return_type psd_vehicleSystem::read(const rclcpp::Time&, const rclcpp::Duration&)
{
  std::shared_ptr<JointState> motor_state;
  received_motor_state_msg_ptr_.get(motor_state);

  RCLCPP_DEBUG(rclcpp::get_logger("psd_vehicleSystem"), "Reading motors state");

  if (!motor_state)
  {
    RCLCPP_ERROR(rclcpp::get_logger("psd_vehicleSystem"), "Feedback message from motors wasn't received");
  ...
  for (auto i = 0u; i < motor_state->name.size(); i++)
  {
    ...
    {
      RCLCPP_ERROR(rclcpp::get_logger("psd_vehicleSystem"), "Position or velocity feedback not found for joint %s",
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("psd_vehicleSystem"), "Position feedback: %f, velocity feedback: %f",
                 pos_state_[motor_state->name[i]], vel_state_[motor_state->name[i]]);
  }
  return return_type::OK;
}

return_type psd_vehicleSystem::write(const rclcpp::Time&, const rclcpp::Duration&)
{
  if (realtime_motor_command_publisher_->trylock())
  {
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("psd_vehicleSystem"), "Wrtiting motors cmd message");
...
PLUGINLIB_EXPORT_CLASS(psd_vehicle_hardware_interfaces::psd_vehicleSystem, hardware_interface::SystemInterface)